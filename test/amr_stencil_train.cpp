...
// Compare the execution time of three versions: CPU, GPU, Apollo
void amr_stencil_rtune_with_tests(const REAL* src, REAL* dst, int* width, int* height, const float* filter, int flt_width, int flt_height) {
    /* width == height == problem size */
    int cpu_exec, iter_count = 0;
...
double gt = read_timer_ms() - elapsed;
...
double ct = read_timer_ms() - elapsed;
	*/
        ...
        elapsed = read_timer_ms();
	Apollo::Region *region = Apollo::instance()->getRegion("stencil", 1, 2);
	region->begin( /* feature vector */ { (float)width[iter_count] } );
	int policy = region->getPolicyIndex();
	switch (policy) {
            case 0:
	      {
		stencil_omp_cpu(src, dst, width[iter_count], height[iter_count], filter, FILTER_WIDTH, FILTER_HEIGHT);
		b_cpu = true;
		break;
	      }
            case 1:
	      {
		stencil_omp_gpu(src, dst, width[iter_count], height[iter_count], filter, FILTER_WIDTH, FILTER_HEIGHT);
		b_gpu = true;
		break;
	      }
