 ...
 *  non-inlined version, GPU version for inner loop only pragmas
 *********************************************************************************/
...
#if SKIP_BACKTRACK
  printf ("skipping the critical section for backtracking ...");
#endif  
  //Allocates a and b
...
//    assert (false);
  ...
  for (i = 1; i <= nDiag; ++i) // start from 1 since 0 is the boundary padding
  {
    ...
    int policy = 2; // pick offloading version always
#endif
    switch (policy)
    {
      case 0:  // serial 
...
#pragma omp parallel for private(j) shared (nEle, si, sj, H, P, maxPos) 
      ...
      case 2: // OpenMP GPU offloading
        ...
        {
          if (i%5000==0)
              printf ("OpenMP GPU version is activated for nEle=  %lld\n", nEle);
          // choice 1: map data before the inner loop

#if 0          // 

#pragma omp target teams map (to:a[0:m-1], b[0:n-1], nEle, m,n,gapScore, matchScore, missmatchScore, si, sj) map(tofrom: H[0:asz], P[0:asz], maxPos)
#pragma omp distribute parallel for default(none) private(j) shared (a,b, nEle, m, n, gapScore, matchScore, missmatchScore, si, sj, H, P, maxPos) 
          for (j = 0; j < nEle; ++j) 
          {  // going upwards : anti-diagnol direction
            long long int ai = si - j ; // going up vertically
            long long int aj = sj + j;  //  going right in horizontal
...
#pragma omp distribute parallel for default(none) shared (a,b, nEle, m, n, gapScore, matchScore, missmatchScore, si, sj, H, P, maxPos)
          ...
          {  // going upwards : anti-diagnol direction
            ...
            int max = NONE;
            int pred = NONE;
            if (diag > max) { //same letter ?~F~V
              max = diag;
              pred = DIAGONAL;
            }

            if (up > max) { //remove letter ?~F~Q
              max = up;
              pred = UP;
            }

            if (left > max) { //insert letter ?~F~P
              max = left;
              pred = LEFT;
            }
...
#if 0
  // explicitly trigger the model building
  Apollo::instance()->flushAllRegionMeasurements(j);
  printf("==========\n DONE TRAINING \n==========\n");  
#endif
